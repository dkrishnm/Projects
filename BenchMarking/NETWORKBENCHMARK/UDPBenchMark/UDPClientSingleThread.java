 ...
 * This class holds the client code for UDP Single thread Network BenchMark
 */
public class UDPClientSingleThread {

    private static String host;
    private static final int PORT = 1234;
    private static int totalSize = 64*1023;
    private static int byteSize = 10;
    ...
    public static void main(String[] args)
    {
        ...
        System.out.println("UDP NETWORK BENCHMARK SINGLE THREAD 64KB");
        ...
        {
            exec.shutdown();
            System.out.println("UDP NETWORK BENCHMARK SINGLE THREAD 1KB");
            ...
            {
                exec1.shutdown();
                System.out.println("UDP NETWORK BENCHMARK SINGLE THREAD 1B");
        ...
        public void run() {
            ...
            try {
                ...
                for (int i = 0; i < numIterations; i++) {
                    ...
                    outPkt = new DatagramPacket(bytes, bytes.length, InetAddress.getByName(host), PORT);
                    dgramSocket.send(outPkt);
                    inPkt = new DatagramPacket(bytes, bytes.length);
                    dgramSocket.receive(inPkt);
                }
                msgOut = "BYE";
                outPkt = new DatagramPacket(msgOut.getBytes(), msgOut.length(), InetAddress.getByName(host), PORT);
